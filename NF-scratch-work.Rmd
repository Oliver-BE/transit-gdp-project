...
## Creating my own df because the other one isn't working 
```{r}
...
## This is the part that goes in the function
```{r}

tree <- rpart(gdp_msa ~ passenger_trips + vehicle_miles + vehicle_hours + total_stations_2017 + total_funding + federal_funding + state_funding + local_funding + other_funding, data=train)
tree
...
## This is our example of our function that won't work 
```{r}

tree_diagram <- function(formula) {
  tree <- rpart(formula, data=train)

pred <- as.vector(predict(tree, test))
test <- test %>% mutate(prediction = pred)

results <- test %>%
  summarise(MSE = 1/n() * sum((outcome_variable - pred)^2, na.rm = T),
            MAE  = 1/n() * sum(abs(outcome_variable - pred), na.rm = T),
            R2   = cor(outcome_variable, pred))
return(results)
}

tree_diagram(as.formula(gdp_msa ~ passenger_trips + vehicle_miles + vehicle_hours + total_stations_2017 + total_funding + federal_funding + state_funding + local_funding + other_funding))

tree_diagram(nicole_df_master$gdp_msa)
