...
```{r}
pop_trips_diagram <- function() {
  train1 <- train #%>% drop_na()
  formula <- paste0("gdp_msa ~ pop_estimate_msa + passenger_trips ")
  ...
  results <- test %>%
    summarise("Explanatory Variables" = "pop_estimate_msa + passenger_trips",
...
pop_trips <- pop_trips_diagram()
pop_trips

```

```{r}
trips_diagram <- function() {
  train1 <- train %>% drop_na("gdp_msa")
  formula <- paste0("gdp_msa ~ passenger_trips")
  tree <- rpart(formula, data=train1)
  ...
  test$truth <- test[,"gdp_msa"]
  results <- test %>%
    summarise("Explanatory Variables" = "passenger_trips",
              MSE = 1/n() * sum((truth - pred)^2, na.rm = T),
              MAE  = 1/n() * sum(abs(truth - pred), na.rm = T),
              R2   = cor(truth, pred))
...
```{r}
table <- rbind(pop_comm_trips, pop_comm_trips_funding)
