...
```{r}
gdp_passengertrips <- ggplot(nicole_df, aes(y = gdp_msa, x = passenger_trips)) + geom_point()
...
gdp_passengertrips_under <- ggplot(nicole_df_gdp_under, aes(y = gdp_msa, x = passenger_trips)) +
  geom_point() +
  geom_smooth(method = lm, se=FALSE)

plot(gdp_passengertrips_under)

gdp_passengertrips_over <- ggplot(nicole_df_gdp_over, aes(y = gdp_msa, x = passenger_trips)) + geom_point()
plot(gdp_passengertrips_over)

```
## Log scale of the above 
```{r}
## ggplots comparing gdp to passenger trips 
nicole_df <- read_csv("../final-data/FINAL_DATA.csv")
trips.x_gdp_under.y <- as.numeric(nicole_df) %>% 
  mutate(log10_trips = log10(passenger_trips)) %>%
  mutate(log10_gdp_under = log10(nicole_df_gdp_under)) %>%
  ggplot(aes(x = log10_trips, y = log10_gdp_under)) + geom_jitter(alpha = .1) + facet_grid(.~transit_modes) + geom_smooth(method = 'lm') 
plot(trips.x_gdp_under.y)

trips.x_gdp_over.y <- nicole_df %>% 
  mutate(log10_trips = log10(passenger_trips)) %>%
  mutate(log10_gdp_over = log10((nicole_df_gdp_over ))) %>%
  ggplot(aes(x = log10_trips, y = log10_gdp_over)) + geom_jitter(alpha = .1) + facet_grid(.~transit_modes) + geom_smooth(method = 'lm') 
plot(trips.x_gdp_over.y)
