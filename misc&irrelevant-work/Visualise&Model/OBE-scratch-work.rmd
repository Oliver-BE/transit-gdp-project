...
test1 <- df %>% 
  filter(msa_id == 16980 | msa_id == 47900 | msa_id == 17460
         | msa_id == 41860 | msa_id == 33100 | msa_id == 35620) %>%  # msa_id == 35620 |) %>% 
...
# gdp vs transit trips
ggplot(test1, aes(x=gdp_msa, y=tot_trips, #size = gdp_msa,
                  color = msa_id_final)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) +
  facet_wrap(~msa_id_final)


# median house hold income vs transit trips
ggplot(test1, aes(x=median_household_income_msa, y=tot_trips, #size = gdp_msa,
                  color = msa_id_final)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) +
  facet_wrap(~msa_id_final)
...
# stuff over time by city
ggplot(test1, aes(x = year, y = tot_fund, col = msa_id_final)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE)
...
m <- leaflet() %>%
  ...
  addMarkers(lng=-73.87376, lat=40.77493, popup="new york") %>% 
