...
gdp_df <- read_xls("/home/class21/obaldwinedwards21/Amherst_classes/STAT231-F18/Final-Project/quality-of-life-data/gdp/gdp-industrytotal.xls", sheet = 1, range = 'A6:S390', col_names=TRUE) %>% 3
...
final_fta_df1 <- left_join(census_UZA_MSA_key, fta_df,
...
final_fta_df <- inner_join(titled_csa_df, final_fta_df1, by = c("msa_id"= "GEO.id2")) %>% 
  group_by(csa_id) %>% 
...
per_capita_df <- final_df %>% 
  mutate(per_capita_total_funding = total_funding/pop_estimate_msa) %>% 
  mutate(per_capita_federal_funding = federal_funding/pop_estimate_msa)%>% 
  mutate(per_capita_state_funding = state_funding/pop_estimate_msa)%>% 
  mutate(per_capita_local_funding = local_funding/pop_estimate_msa)%>% 
  mutate(per_capita_other_funding = other_funding/pop_estimate_msa)%>%
  mutate(per_capita_passenger_trips = passenger_trips/pop_estimate_msa) %>% 
  mutate(per_capita_vehicle_miles = vehicle_miles/pop_estimate_msa) %>% 
  mutate(per_capita_vehicle_hours = vehicle_hours/pop_estimate_msa) 
