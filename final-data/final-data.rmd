...
metropolitan_coordinates <- read_xlsx("/home/class21/obaldwinedwards21/Amherst_classes/STAT231-F18/Final-Project/quality-of-life-data/latitude-longitude/StatisticalAreasLatLong.xlsx", sheet = 1,
                                      range = "A2:Q376", col_names = TRUE)
# coordinates for micropolitan areas
micropolitan_coordinates <- read_xlsx("/home/class21/obaldwinedwards21/Amherst_classes/STAT231-F18/Final-Project/quality-of-life-data/latitude-longitude/StatisticalAreasLatLong.xlsx", sheet = 2,
...
clean_fta_df <- function(sheet_number, col_name) {
  ...
  df <- read_xlsx("/home/class21/obaldwinedwards21/Amherst_classes/STAT231-F18/Final-Project/FTA-data/FTA-dataframes/FTA_September_2018.xlsx", sheet=sheet_number, range="A1:HB2129", col_names=TRUE) 
...
fta_df <- full_join(final_master_fta_df, upt_df, by= c("UZA")) %>% 
  ...
  full_join(transit_stations_df, by = "UZA") %>% 
...
census_UZA_MSA_key <- read_csv("/home/class21/obaldwinedwards21/Amherst_classes/STAT231-F18/Final-Project/census-FTA-joining-key/UZA_MSA_join.txt") %>% 
  mutate("GEO.id2" = CBSA) %>% 
  arrange(desc(UAPOP)) %>% 
...
# clarify variables names
  colnames(final_df)[1:30] <- c("msa_id", 
                               "msa_name", 
                               "pop_estimate_msa", 
                               "year", 
  ...
  final_df <- final_df %>% 
    select(-c("ua_census_name", "ua_fta_id"))
